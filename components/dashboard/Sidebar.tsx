...
import { LayoutDashboard, BookOpen, FileText, Brain, Upload, BarChart3, Users, Settings, LogOut, User } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ 
  isOpen, activeTab, userRole, onTabChange, onRoleChange, onLogout 
}) => {
  const visibleItems = SIDEBAR_ITEMS.filter(item => item.roles.includes(userRole));

  return (
    <aside className={`${isOpen ? 'w-72' : 'w-24'} bg-[#0a0f1e] text-slate-300 flex flex-col flex-shrink-0 transition-all duration-300 border-r border-white/5`}>
      {/* Brand */}
      <div className="p-8 border-b border-white/5 flex items-center gap-4">
        <img 
           src="https://res.cloudinary.com/dsvjhemjd/image/upload/v1764105292/yellow_logo_png_bvs11z.png" 
           alt="Ryze" 
           className="h-8 w-auto" // Original gold logo works well on dark
        />
      ...
      {/* User Context */}
      <div className="p-6">
        <div className={`bg-white/5 rounded-2xl p-4 flex items-center gap-4 mb-8 border border-white/5 ${!isOpen && 'justify-center'}`}>
        ...
        <nav className="space-y-2">
          {visibleItems.map((item) => (
            ...
            >
              <item.icon size={22} className={item.highlight && activeTab !== item.id ? 'text-[#FFB000]' : ''} />
              {isOpen && <span>{item.label}</span>}
              {!isOpen && (
                <div className="absolute left-full ml-4 bg-[#1e293b] text-white px-3 py-1.5 rounded-lg text-xs opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 shadow-xl border border-white/10 font-bold tracking-wide">
                  {item.label}
                </div>
              )}
            </button>
