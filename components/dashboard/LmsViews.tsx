    ...
    {courses.map(course => (
      <div key={course.id} className="bg-[#0a0f1e] p-8 rounded-3xl border border-white/5 shadow-lg hover:border-[#FFB000]/50 transition-all group">
         <div className="flex justify-between items-start mb-6">
            ...
            <span className="text-[10px] font-bold bg-white/5 ryze-text-muted px-3 py-1.5 rounded-full border border-white/5 tracking-wider">{course.code}</span>
         </div>
         <h3 className="font-bold text-xl ryze-text-inverse mb-4 leading-tight">{course.title}</h3>
         ...
         <div className="flex justify-between text-xs ryze-text-muted font-medium">
...
export const AssignmentsList: React.FC<{ assignments: Assignment[] }> = ({ assignments }) => (
  <div className="bg-[#0a0f1e] rounded-[2rem] border border-white/5 overflow-hidden shadow-lg">
      <div className="grid grid-cols-12 gap-4 px-8 py-5 bg-white/5 border-b border-white/5 text-xs font-bold ryze-text-muted uppercase tracking-widest">
      ...
      {assignments.map(a => (
         <div key={a.id} className="grid grid-cols-12 gap-4 px-8 py-6 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors items-center group">
             <div className="col-span-6 flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center ryze-text-muted group-hover:text-[#FFB000] transition-colors">
                   <FileText size={18} />
                </div>
                <span className="font-medium text-slate-200 group-hover:ryze-text-inverse transition-colors text-base">{a.title}</span>
             </div>
             <div className="col-span-3 text-sm ryze-text-muted flex items-center gap-2">
