  ...
  <div className="space-y-8 animate-pulse">
     ...
     <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[#0a0f1e] p-6 rounded-2xl border border-white/5 h-96 relative overflow-hidden">
           <div className="absolute top-4 right-4 ryze-text-inverse-muted"><FileJson size={80} /></div>
    ...
    <div className="h-[calc(100vh-140px)] min-h-[600px] flex flex-col max-w-6xl mx-auto bg-[#0a0f1e] rounded-[2.5rem] border border-white/5 shadow-2xl overflow-hidden relative">
      ...
      <div className="p-4 md:p-8 border-b border-white/5 bg-[#0a0f1e] flex flex-col md:flex-row justify-between items-center gap-4 z-20 relative">
         <div className="flex items-center gap-4">
            ...
            <div>
               <h2 className="font-bold ryze-text-inverse text-base md:text-lg">Ryze AI Engine</h2>
               <div className="text-xs ryze-text-muted flex items-center gap-2 mt-0.5">
         ...
         <div className="flex bg-black/20 p-1.5 rounded-xl border border-white/5 w-full md:w-auto">
            <button 
              ...
              className={`flex-1 md:flex-none px-5 py-2.5 text-xs font-bold rounded-lg transition-all whitespace-nowrap ${mode === 'chat' ? 'bg-[#FFB000] text-[#0a0f1e] shadow-lg' : 'ryze-text-muted hover:ryze-text-inverse hover:bg-white/5'}`}
            >
              Tutor Chat
            </button>
            <button 
              onClick={() => setMode('generator')}
              className={`flex-1 md:flex-none px-5 py-2.5 text-xs font-bold rounded-lg transition-all whitespace-nowrap ${mode === 'generator' ? 'bg-[#FFB000] text-[#0a0f1e] shadow-lg' : 'ryze-text-muted hover:ryze-text-inverse hover:bg-white/5'}`}
            >
              Question Generator
            </button>
         </div>
      ...
      {mode === 'chat' && (
        <div className="flex-1 flex flex-col relative bg-[#050510] overflow-hidden">
          ...
          {chatHistory.length > 0 && (
            <div className="absolute top-4 right-4 z-10">
              <button 
                ...
                className="p-3 bg-[#0a0f1e] ryze-text-muted hover:text-red-400 hover:bg-red-500/10 hover:border-red-500/30 rounded-full shadow-lg border border-white/5 transition-all group"
                title="Clear Chat History"
              >
                <Trash2 size={18} />
                <span className="absolute right-full mr-3 top-1/2 -translate-y-1/2 bg-black ryze-text-inverse text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-white/10 hidden md:block">Clear History</span>
              </button>
            </div>
          )}
          ...
          <div className="flex-1 p-4 md:p-8 overflow-y-auto space-y-6 md:space-y-8 scroll-smooth">
             {chatHistory.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full ryze-text-muted text-center px-4">
                   ...
                   <p className="text-base md:text-lg font-medium ryze-text-muted">Start a conversation with Ryze AI Tutor.</p>
                   <p className="text-xs md:text-sm ryze-text-secondary mt-2">Ask about calculus, essay structure, or physics concepts.</p>
                </div>
             )}
             {chatHistory.map((msg, idx) => (
               <div key={idx} className={`flex gap-3 md:gap-5 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                 <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center flex-shrink-0 mt-1 text-[10px] md:text-xs font-bold shadow-lg ${
                    ...
                    : 'bg-[#0a0f1e] ryze-text-inverse border border-white/10'
                 }`}>
                    {msg.role === 'user' ? 'U' : 'AI'}
                 </div>
                 <div className={`p-4 md:p-5 rounded-2xl md:rounded-3xl text-sm md:text-[15px] leading-relaxed shadow-md max-w-[80%] md:max-w-2xl break-words ${
                    ...
                    : 'bg-[#0a0f1e] border border-white/10 ryze-text-inverse-muted rounded-tl-none'
                 }`}>
                    {msg.text}
                 </div>
               </div>
             ...
             {isTyping && (
                <div className="flex gap-3 md:gap-5">
                   <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#0a0f1e] ryze-text-inverse flex items-center justify-center flex-shrink-0 mt-1 text-[10px] md:text-xs font-bold border border-white/10">AI</div>
          ...
          <div className="p-4 md:p-6 bg-[#0a0f1e] border-t border-white/5">
            <div className="relative max-w-4xl mx-auto flex items-center gap-2">
                <input 
                  ...
                  className="flex-1 pl-4 md:pl-6 pr-12 py-3 md:py-5 rounded-xl md:rounded-2xl bg-[#050510] border border-white/10 ryze-text-inverse placeholder-slate-500 focus:outline-none focus:border-[#FFB000]/50 focus:ring-1 focus:ring-[#FFB000]/50 transition-all text-base md:text-lg shadow-inner"
                />
                <button 
                  onClick={handleSend}
                  disabled={!input.trim() || isTyping}
                  className="p-3 md:p-4 bg-[#FFB000] rounded-xl text-[#0a0f1e] hover:bg-[#ffc133] disabled:opacity-50 disabled:bg-slate-700 disabled:ryze-text-muted transition-all flex-shrink-0"
                >
              ...
              {!generatedContent && !isGenerating ? (
                 <div className="bg-[#0a0f1e] p-6 md:p-10 rounded-[2rem] shadow-xl border border-white/5 relative overflow-hidden">
                    ...
                    <div className="text-center mb-10 relative z-10">
                       ...
                       <h3 className="text-2xl md:text-3xl font-bold ryze-text-inverse mb-2">Content Generator</h3>
                       <p className="text-sm md:text-base ryze-text-muted">Generate exam-style questions with real-time curriculum grounding.</p>
                    </div>
                    
                    <div className="space-y-6 mb-10 relative z-10">
                       <div>
                          <label className="block text-xs font-bold ryze-text-muted uppercase mb-3 tracking-widest">Topic</label>
                          <div className="relative">
                              <select 
                                value={genConfig.topic}
                                onChange={(e) => setGenConfig({...genConfig, topic: e.target.value})}
                                className="w-full p-4 pr-10 rounded-xl border border-white/10 bg-[#050510] ryze-text-inverse focus:border-[#FFB000] focus:ring-1 focus:ring-[#FFB000] outline-none transition-all appearance-none cursor-pointer text-sm md:text-base"
                              >
                                {TOPICS.map(t => <option key={t} value={t}>{t}</option>)}
                              </select>
                              <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 ryze-text-muted pointer-events-none" size={18} />
                          </div>
                       </div>
                       <div>
                          <label className="block text-xs font-bold ryze-text-muted uppercase mb-3 tracking-widest">Difficulty</label>
                          <div className="relative">
                              <select
                 ...
                 <div className="space-y-8">
                    <div className="flex justify-between items-center">
                       <h3 className="font-bold text-xl md:text-2xl ryze-text-inverse">Generated Output</h3>
                       <button 
                          onClick={() => { setGeneratedContent(null); setIsGenerating(false); }} 
                          disabled={isGenerating}
                          className="flex items-center gap-2 text-xs md:text-sm text-[#FFB000] font-bold hover:ryze-text-inverse transition-colors bg-[#FFB000]/10 px-3 py-2 md:px-4 md:py-2 rounded-lg border border-[#FFB000]/20 disabled:opacity-50"
                       >
                    ...
                    {isGenerating ? (
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                          ...
                          <div className="bg-[#0a0f1e] p-6 rounded-2xl border border-white/5 shadow-lg relative overflow-hidden h-96 hidden md:block">
                            <div className="absolute top-4 right-4 opacity-5 ryze-text-inverse"><FileJson size={80} /></div>
                            <h4 className="text-xs font-bold ryze-text-muted uppercase tracking-widest mb-4">Raw JSON</h4>
                            <pre className="text-xs font-mono ryze-text-muted overflow-auto h-[300px] scrollbar-thin scrollbar-thumb-slate-700">
                          ...
                          <div className="bg-[#0a0f1e] p-6 md:p-8 rounded-2xl border border-white/5 shadow-lg flex flex-col h-auto">
                            ...
                            <p className="font-medium ryze-text-inverse mb-8 text-lg md:text-xl leading-relaxed">{generatedContent.stem}</p>
                            
                            <div className="bg-[#050510] p-6 rounded-xl text-sm ryze-text-inverse-muted border border-white/5 mb-6">
                            ...
                            {generatedContent.sources && generatedContent.sources.length > 0 && (
                                <div className="mt-auto pt-6 border-t border-white/5">
                                  <p className="text-xs font-bold ryze-text-muted uppercase mb-3 tracking-widest">Context Sources</p>
